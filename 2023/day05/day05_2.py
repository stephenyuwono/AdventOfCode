...
maps[keys[0]] = maps[keys[0]].reshape(len(maps[keys[0]])//2,2)

# maps[keys[0]][:,1] += maps[keys[0]][:,0] - 1
maps[keys[0]][:,1] += maps[keys[0]][:,0]

for i in maps[keys[1]][:,1]:
    print(i)
    for x in range(10):
        if maps[keys[0]][x,0] < i < maps[keys[0]][x,1]:
            print(f'yes, {maps[keys[0]][x,0]}')
        else:
            continue
            # print(f'no, {maps[keys[0]][x,0]}')
